...
FireTimer loraTimer, serialTimer;
...
void loop() {
        g_fisInput[0] = ultrasonicValue;   // Read Input: Ketingian_Air
        g_fisInput[1] = turbidityValue;    // Read Input: Kekeruhan_Air
        g_fisInput[2] = temperatureValue;  // Read Input: Suhu_Air
        g_fisInput[3] = pHValue;           // Read Input: pH_Air
        g_fisInput[4] = tdsValue;          // Read Input: Zat_Padat_Terlarut_Pada_Air
        ...
        if (serialTimer.fire()) {
                if (stateMode == 1) {
                        Serial.print(statePengisian);  // Set output value: Pengisian
                        Serial.print(" ");
                        Serial.print(statePengurasan);  // Set output value: Pengurasan
                        Serial.print(" ");
                        Serial.print(stateHeater);  // Set output value: Heater
                        Serial.print(" ");
                        Serial.print(statePhDown);  // Set output value: Cairan_Asam
                        Serial.print(" ");
                        Serial.print(statePhUp);  // Set output value: Cairan_Basa
                        Serial.println();
                } else {
                        Serial.print(g_fisOutput[0]);  // Set output value: Pengisian
                        Serial.print(" ");
                        Serial.print(g_fisOutput[1]);  // Set output value: Pengurasan
                        Serial.print(" ");
                        Serial.print(g_fisOutput[2]);  // Set output value: Heater
                        Serial.print(" ");
                        Serial.print(g_fisOutput[3]);  // Set output value: Cairan_Asam
                        Serial.print(" ");
                        Serial.print(g_fisOutput[4]);  // Set output value: Cairan_Basa
                        Serial.println();
                }
        }
        ...
        if (packetSize) {
                ...
                stateMode = parseString(data, " ", 5).toFloat();
        }
        if (loraTimer.fire()) {
                String loraData;
                loraData += String(ultrasonicValue) + " ";
                loraData += String(turbidityValue) + " ";
                loraData += String(temperatureValue) + " ";
                loraData += String(pHValue) + " ";
                loraData += String(tdsValue) + " ";
...
FIS_TYPE fis_gMFI0Coeff1[] = { 0, 0, 75, 85 };
FIS_TYPE fis_gMFI0Coeff2[] = { 80, 90, 100 };
FIS_TYPE fis_gMFI0Coeff3[] = { 95, 105, 200, 200 };
...
FIS_TYPE fis_gMFI3Coeff1[] = { 0, 0, 6, 6.5 };
FIS_TYPE fis_gMFI3Coeff2[] = { 6, 7, 8 };
FIS_TYPE fis_gMFI3Coeff3[] = { 7.5, 8, 14, 14 };
